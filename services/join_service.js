    ...
    try {
        ...
        if (targetUser.length > 0) {
            const notifMetadata = JSON.stringify({
                message: `You have been accepted by the host to join ${events[0].title}.`,
                event_title: events[0].title,
                event_type,
                event_id
            });
            await sequelize.query(
                ...
                { replacements: [parts[0].user_id, `approve_${event_id}_${Date.now()}`, notifMetadata], transaction: t }
            ).catch(err => console.error("[JoinNotif] DB Insert Error:", err));
        }
...
router.post('/join/reject', async (req, res) => {
    ...
    try {
        ...
        if (targetUser.length > 0) {
            const notifMetadata = JSON.stringify({
                message: `Your request to join ${events[0].title} was declined by the host.`,
                event_title: events[0].title,
                event_type,
                event_id
            });
            await sequelize.query(
                ...
                { replacements: [parts[0].user_id, `reject_${event_id}_${Date.now()}`, notifMetadata], transaction: t }
            ).catch(err => console.error("[JoinNotif] DB Insert Error:", err));
        }
    ...
    try {
        const query = `
            ...
            SELECT 
                ...
                'host' as user_role,
                (SELECT COUNT(*) FROM event_participants ep JOIN users u_ghost ON ep.user_id = u_ghost.id WHERE LOWER(ep.event_type) = 'housing' AND ep.event_id = ho.id AND LOWER(ep.status) IN ('approved', 'accepted') AND LOWER(u_ghost.email) != 'ncnujoinupadmin@gmail.com') as approvedCount
            FROM housing ho
